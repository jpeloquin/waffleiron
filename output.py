    ...
    for axis, nodeset in model.fixed['node'].items():
        if nodeset:
            if version == '2.0':
                ...
                nm = nm_base
                i = 0
                while nm in model.named_sets:
                    nm = nm_base + "_" + str(i)
                model.named_sets['nodes'][nm] = nodeset
                # Write the autogenerated nodeset's XML tags
                e_fixed_nodeset = ET.SubElement(Geometry, "NodeSet", name=nm)
                # Create the tag
                ET.SubElement(e_boundary, 'fix', bc=febioxml.axis_to_febio[axis],
                              node_set=nm)
            # Create a tag for each node
            for nid in nodeset:
                ET.SubElement(e_fixed_nodeset, 'node', id=str(nid + 1))
