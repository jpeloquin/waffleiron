...
# As of FEBio XML 4.0, body conditions are now split between <rigid_bc> and
...
def read_body_bcs(
    ...
    body_constraints = []
    for tag in ["rigid_bc", "rigid_load"]:
        for e_rbc in root.findall(f"{BODY_COND_PARENT}/{tag}"):
...
def read_body_bc(
    ...
    mat_name = find_unique_tag(e_rbc, "rb").text
    ...
    if e_rbc.tag == "rigid_bc":
        if e_rbc.attrib["type"] == "prescribe":
            var = "displacement"
        ...
        elif e_rbc.attrib["type"] == "fix":
            xml_dofs = (
                s.strip() for s in find_unique_tag(e_rbc, "dofs").text.split(",")
            )
            for xml_dof in xml_dofs:
                dof = DOF_FROM_XML_RB_DOF[xml_dof]
                var = VAR_FROM_XML_RB_DOF[xml_dof]
                constraints.append(BodyConstraint(body, dof, var, True, None, False))
    elif e_rbc.tag == "rigid_load":
        # Despite the name, <rigid_force> includes moments
        if e_rbc.attrib["type"] == "rigid_force":
            var = "force"
            dof = DOF_FROM_XML_RB_DOF[find_unique_tag(e_rbc, "dof").text]
            seq = read_parameter(find_unique_tag(e_rbc, "value"), sequences)
            constraints.append(BodyConstraint(body, dof, var, False, seq, False))
            if e_rbc.find("load_type").text not in ("0", "LOAD"):
                raise NotImplementedError()
            # TODO: Follower load
            # TODO: "Target" load (syntactic sugar for autogenerated ramp)
        else:
            raise NotImplementedError()
    return constraints
